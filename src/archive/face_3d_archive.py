...
class Face3D:
    ...
    def update(self, *vertices):
        ...
        self.vertices_xy = [linalg.proj_2_xy(vertex) for vertex in self.vertices]


    def to_2D(self, plane_normal):
        return [linalg.proj_2_plane(vertex, plane_normal) for vertex in self.vertices]

    def set_render_colours(self, cfill):
        self.cfill = cfill

    def rotate(self, angle, rot_axis):
        vertices = [linalg.rotate_vec(vertex, angle, rot_axis)
